...
Not the length of the sequence of directions, the length of the ICFP expression you send to the server. That means for repetitive strings it's better to send a program that evaluates to the needed string, rather than just the string literal itself.
...
This problem is an instance of [Kolmogorov complexity](https://en.wikipedia.org/wiki/Kolmogorov_complexity), which is actually equivalent to the halting problem, so no perfect solution is possible.
...
$$\max(\left\vert x_0 + x_2 - x_1 \right\vert, \left\vert y_0 + y_2 - y_1 \right\vert)$$
